...
s2 as (
    SELECT
        ...
        customer_id,
    coalesce(tax_id, last_value(tax_id) OVER (
            PARTITION BY customer_id
            ORDER BY effective_timestamp)) tax_id,
        status,
    coalesce(last_name, last_value(last_name) OVER (
            PARTITION BY customer_id
            ORDER BY effective_timestamp)) last_name,
    coalesce(first_name, last_value(first_name) OVER (
            PARTITION BY customer_id
            ORDER BY effective_timestamp)) first_name,
    coalesce(middle_name, last_value(middle_name) OVER (
            PARTITION BY customer_id
    ...
    coalesce(address_line1, last_value(address_line1) OVER (
            PARTITION BY customer_id
            ORDER BY effective_timestamp)) address_line1,
    coalesce(address_line2, last_value(address_line2) OVER (
            PARTITION BY customer_id
            ORDER BY effective_timestamp)) address_line2,
    coalesce(postal_code, last_value(postal_code) OVER (
            PARTITION BY customer_id
            ORDER BY effective_timestamp)) postal_code,
    coalesce(CITY, last_value(CITY) OVER (
            PARTITION BY customer_id
            ORDER BY effective_timestamp)) CITY,
    coalesce(state_province, last_value(state_province) OVER (
            PARTITION BY customer_id
    ...
    coalesce(primary_email, last_value(primary_email) OVER (
            PARTITION BY customer_id
            ORDER BY effective_timestamp)) primary_email,
    coalesce(alternate_email, last_value(alternate_email) OVER (
            PARTITION BY customer_id
            ORDER BY effective_timestamp)) alternate_email,
    coalesce(local_tax_rate_name, last_value(local_tax_rate_name) OVER (
            PARTITION BY customer_id
            ORDER BY effective_timestamp)) local_tax_rate_name,
    coalesce(local_tax_rate, last_value(local_tax_rate) OVER (
            PARTITION BY customer_id
            ORDER BY effective_timestamp)) local_tax_rate,
    coalesce(national_tax_rate_name, last_value(national_tax_rate_name) OVER (
            PARTITION BY customer_id
            ORDER BY effective_timestamp)) national_tax_rate_name,
    coalesce(national_tax_rate, last_value(national_tax_rate) OVER (
            PARTITION BY customer_id
